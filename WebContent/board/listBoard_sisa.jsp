					...
					<c:forEach var="board" items="${requestScope.boards}" varStatus="loop">
						...
						<tr>
							...
							<c:if test="${pageScope.board.boardNotice==1 }">
								<td><a href="${pageScope.url}" class="notice">${pageScope.board.boardTitle }</a>
								</td>
							</c:if>
							<c:if test="${pageScope.board.boardNotice==0 }">
								<td><a href="${pageScope.url}">${pageScope.board.boardTitle }</a>
								</td>
							</c:if>
	...
	<div class="bottom">
		...
		<div id="paging">
			...
			<c:if test="${startPage > pageBlock }">
				<c:url var="prevUrl" value="/listBoardsisa.do">
					<c:param name="cateNo" value="${param.cateNo }"></c:param>
			...
			<c:forEach var="i" begin="${startPage}" end="${endPage}">
				...
				<c:if test="${i != currentPage}">
					<c:url var="url" value="/listBoardsisa.do">
						<c:param name="cateNo" value="${param.cateNo }"></c:param>
			...
			<c:if test="${endPage < totalPage}">
				<c:url var="nextUrl" value="/listBoardsisa.do">
		...
		<c:url var="writeUrl" value="/previewWriteForm.do">
			<c:param name="cateNo" value="${param.cateNo}"></c:param>
		</c:url>
