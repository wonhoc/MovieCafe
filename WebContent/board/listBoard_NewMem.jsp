	...
	<div class="list_top">
		<h3 class="list_title">새싹 게시판</h3>
			...
			<div id="search">
				...
				<input type="search" placeholder="검색어를 입력하세요" size="30"
					...
					<c:forEach var="board" items="${requestScope.boards}"
						...
						<tr>
							...
							<c:if test="${pageScope.board.boardNotice==1 }">
								<td><a
									href="${pageScope.url}" class="notice">${pageScope.board.boardTitle }</a></td>
							</c:if>
	...
	<div class="bottom">
		...
		<div id="paging">
			...
			<c:if test="${startPage > pageBlock }">
				<c:url var="prevUrl" value="/listBoardNewMem.do">
					<c:param name="cateNo" value="${param.cateNo }"></c:param>
			...
			<c:if test="${endPage < totalPage}">
				<c:url var="nextUrl" value="/listBoardNewMem.do">
		...
		<c:url var="writeUrl" value="/rankupWriteBoardForm.do">
			<c:param name="cateNo" value="${param.cateNo}"></c:param>
		</c:url>
